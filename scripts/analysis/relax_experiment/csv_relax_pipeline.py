...
def extract_ratios(graph, considered_energies, penalizer):
    ...
    considered_ratios = [
        clas.approx_ratio(graph, energy, penalizer, theo_best_cost) 
                if energy is not None else None
                for energy in considered_energies
    ]
    return considered_ratios

def compare_pipeline(warm_qaoa_run, cold_qaoa_run=None, graph=None, N=None, seed=0):
    if graph is None:
        graph = warm_qaoa_run["problem"]["graph"]
    if N is None:
        N = warm_qaoa_run["problem"]["N"]
    penalizer = 1.5
